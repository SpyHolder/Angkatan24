<x-main-layout>
    ...
    <div class="flex justify-center gap-10 py-10 border-b-2 border-black">
        {{-- Content --}}
        <a href="{{ route('detail-news', $data->first()->news_id) }}" class="group ">
            <div class="w-full md:px-0 md:w-[700px] animate-fade-right animate-duration-1000 animate-delay-[600ms]">
                <div class="px-2">
                    <div class="overflow-hidden rounded-2xl">
                        <img src="{{ asset('img/news/' . $data->first()->picture_news) }}" alt="Berita Eksklusif"
                            class="object-fill group-hover:scale-110 transition-transform duration-200 ease-in-out">
                    </div>
                </div>
                <div class="px-2 mt-2">
                    <p class="text-right text-xs md:text-base">{{ $data->first()->login->username }},
                        {{ $data->first()->covarage_area }}
                        | {{ $data->first()->date_publish->translatedFormat('d F Y') }},
                        {{ $data->first()->time_publish }}
                    </p>
                    <p class="text-xl md:text-3xl font-bold mt-4 group-hover:underline px-2">{{ $data->first()->headline_news }}</p>
                    <p class="text-sm md:text-lg mt-2 line-clamp-3 px-2">
                        {{ str_replace('&nbsp;', ' ', strip_tags($data->first()->content_news)) }}</p>
                </div>
            </div>
